...
public class KakaocertServiceController {
    ...
    public String requestESign(Model m) {
        ...
        // 수신자 휴대폰번호
        request.setReceiverHP("01012341324");
         ...
         * 
         * - 인증메시지를 수신한 사용자가 카카오인증 비회원일 경우, 카카오인증 회원등록 절차를 거쳐 은행계좌 실명확인 절차이후 전자서명 가능
         */
        request.setAllowSimpleRegistYN(false);

        /*
         * 수신자 실명확인 여부 true : 카카오페이가 본인인증을 통해 확보한 사용자 실명과 ReceiverName 값을 비교 false :
         * 카카오페이가 본인인증을 통해 확보한 사용자 실명과 RecevierName 값을 비교하지 않음.
         */
